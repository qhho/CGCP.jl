...
function evaluate_policy(m, policy, simmer::RolloutSimulator; parallel=true)
    ...
    if parallel && Threads.nthreads() > 1
        Threads.@threads for i in 1:n_sim
            v, c = ConstrainedPOMDPs.simulate(simmer, m, policy, updater(policy), initialstate(m), rand(initialstate(m)))
            total_v += v
            total_c += c
        end
    else
...
function compute_policy(m::ConstrainedPOMDPWrapper, solver, λ::Float64, τ::Float64, ρ::Float64)
    m.λ = λ
    solver.timeout = τ
